...
static int disp_gamma_write_lut_reg(cmdqRecHandle cmdq, disp_gamma_id_t id, int lock)
{
	...
	if (gamma_lut == NULL) {
		pr_notice("[GAMMA] disp_gamma_write_lut_reg: gamma table [%d] not initialized\n", id);
		ret = -EFAULT;
	...
	if (id == DISP_GAMMA0) {
		if (DISP_REG_GET(DISP_AAL_EN) == 0) {
			pr_warn("[GAMMA][WARNING] DISP_AAL_EN not enabled!\n");
...
static int disp_ccorr_write_coef_reg(cmdqRecHandle cmdq, disp_ccorr_id_t id, int lock)
{
	...
	if (ccorr == NULL) {
		pr_notice("[GAMMA] disp_ccorr_write_coef_reg: [%d] not initialized\n", id);
		ret = -EFAULT;
	...
	if (id == DISP_CCORR0) {
		...
		pr_err("[GAMMA] disp_gamma_write_ccorr_reg: invalid ID = %d\n", id);
		ret = -EFAULT;
		goto ccorr_write_coef_unlock;
	}
